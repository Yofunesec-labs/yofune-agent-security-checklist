...
**Document type:** Technical whitepaper, verification methodology, and public-draft conformance model
...
The project currently defines **10 security domains, 52 core controls, 58 reusable verification tests, 8 architecture profiles, YAL-0 through YAL-4 assurance levels, a verification-run format, and an assurance-case format**.
...
For material assessments, YASC v0.5 introduces a machine-readable **Verification Plan**. The plan fixes the system boundary, applicable controls, test set, environment, trial policy, security oracles, evidence policy, safety constraints, and reviewer independence before results are interpreted. This reduces post-hoc changes to pass criteria and makes repeated assessments comparable.
...
Agent workflows are increasingly asynchronous. Authorization can be correct when checked and wrong when the side effect occurs. YASC v0.5 therefore adds explicit tests for time-of-check/time-of-use mutation, approval expiry, revocation propagation, delayed queues, retry resurrection, and service restart.
...
### 16.2 Execution adapters

The v0.5 repository also introduces a small framework-neutral harness. It can validate plans, scaffold verification runs from YAT definitions, hash evidence packages, verify evidence manifests, and report plan-to-run coverage. Target-specific execution remains an adapter responsibility because YASC should not hard-code one agent framework or assume authorization to attack arbitrary systems.
...
YASC v0.5 adds a public-draft conformance claim format. It distinguishes **control conformance**, **profile conformance**, and a **baseline assessment**. Status and assurance remain separate: `PASS` means the pass criteria were met in scope; `YAL-4` describes the strength of the evidence. A failed critical control is never averaged away by other passes.

Every shared claim should identify YASC version, exact system state, environment, included and excluded paths, profiles, per-control result, achieved YAL, evidence/assurance case, known gaps, validity window, revalidation triggers, and reviewer independence. v0.5 does not authorize the phrase “YASC certified secure.”
...
After v0.5, YASC work should prioritize executable adapters and reviewer feedback while preserving the stable control IDs. Research areas include:
...
- `schema/conformance-claim.schema.json` — bounded public-draft conformance claim;
...
- `scripts/yasc_harness.py` — plan/run/evidence/coverage utilities;
