...
- Agent / MCP Adapter；
- Evidence Collector 与实际 Test Runner；
...
对重要评估，v1.0 定义机器可读的 **Verification Plan**。在运行测试之前固定 System Boundary、Applicable Control、Test Set、Environment、Trial Policy、Security Oracle、Evidence Policy、安全约束以及 Reviewer Independence。这样可以减少“看到结果以后再修改 Pass Criteria”的偏差，也让不同版本的评估可以比较。
...
v1.0 定义机器可读 **Evidence Manifest**。每个证据 Artifact 可以记录 Type、Path、SHA-256、Size、Source Component、Trace ID、Collection Time、Sensitivity 和 Redaction Notes。Hash 不能证明 Source System 本身没有撒谎，但能让收集后的文件被修改时可检测。
...
### 16.2 v1.0 可执行 Verification Harness

v1.0 把 Harness 从“Run Scaffold 工具”升级成真正可以执行的参考实现。YAT 定义仍然负责描述 Test Purpose、Procedure、Expected Result、Evidence 和 Safety；**Execution Scenario** 则把某个 YAT 绑定到具体 Target 的 Adapter Action 与机器可判定 Assertion。这样可以自动化测试，而不需要为了某个 Agent Framework 修改标准本身。

参考实现包含：

- deterministic mock Agent Adapter，用于仓库自测；
- 可配置字段映射的 JSON-over-HTTP Agent Adapter；
- OpenAI Responses API Adapter，记录模型提出的 `function_call`，但默认不替目标执行工具；
- Anthropic Messages API Adapter，记录客户端侧 `tool_use`，但默认不替目标执行工具；
- LangGraph 进程内 Adapter，可导入具有 `.invoke()` 的 compiled graph / runnable；
- CrewAI 进程内 Adapter，可导入 Crew / factory 并调用 `.kickoff()`；
- MCP stdio / HTTP Adapter，可在现代 `2026-07-28` discovery/stateless 协议与 legacy initialize/session 行为之间自动协商，并支持 `tools/list`、`tools/call`；
- 覆盖全部 58 个 YAT ID 的 Scenario Registry；
- Trial 重复执行与 State Reset；
- 标准化 Verification Run 输出。

Provider Adapter 有意把**工具调用提议的观测**与**工具实际执行**分开。这样在对抗验证中，如果模型提出了被禁止的高风险动作，YASC 可以把该提议作为证据和判定输入，而不会因为 Harness 自己执行工具而制造真实副作用。工具执行仍属于目标系统或经过明确约束的 Target-specific Fixture。MCP Transport 则根据协议代际处理：现代 Target 使用 discovery、per-request metadata 与 stateless routing；旧版 Target 保留 initialize/session 协商。

不是所有 YAT 都被虚假标记成“全自动”。Tenant Boundary、Credential Revocation、Memory Lifecycle、Queue、Network Egress、Supply-chain Rollback、Fault Injection、Kill Switch 等测试必须结合目标系统真实 Fixture 与 Telemetry，因此默认保持 `guided`，由目标团队提供 Target-specific Scenario。

### 16.3 Evidence Collector

每次执行都可以创建独立 Evidence Directory，记录 JSONL Event、Request/Response、Assertion Result 和 Runner Error。参考 Collector 在写盘前会对常见 Authorization、Token、API Key、Password、Cookie、Credential 字段做默认 Redaction，并掩码 Bearer Token。这个能力是 Guardrail，不等同于完整 DLP；Assessment Owner 仍需负责 Evidence Classification、Redaction Review、Retention 与访问控制。

Collector 最终生成 SHA-256 Evidence Manifest。CI Gate 可以重新计算所有 Artifact Hash，发现缺失或被修改的证据。Hash 能证明“收集后的 Bytes 没被改”，不能证明 Source Telemetry 本身一定真实完整。

### 16.4 实际 Test Runner 与 Assessment Builder

Test Runner 读取 Verification Plan、Target Config 和 Scenario，调用 Agent/MCP Adapter 执行 Step、计算 Assertion、记录 Trial Failure，并输出 `verification-run`。随后 Assessment Builder 只基于 Plan 中明确列出的测试推导逐 Control Status 与 bounded Assurance，避免把未测试路径自动算成 PASS。
...
v1.0 可以从 Plan + Run + Evidence + Gate Result 生成 Yofune 品牌 HTML/PDF Assessment Report。报告包含 Target、Scope、Coverage、Test Verdict、Mapped Control、Evidence Reference 与 Gate Status。PDF 是 Release/Communication Artifact；机器可读 Run 和 Evidence 才是复核真源。

### 16.6 CI Security Gate

CI Gate 可以配置 Minimum Test Coverage、Required Test、按 Severity 阻断失败、Maximum Inconclusive、Evidence Manifest Integrity 与 Evidence Freshness。Gate Fail 返回非零 Exit Code，适合直接阻断 Pull Request 或 Release Pipeline。

Gate PASS 只说明当前 Plan、Target State、Environment、Scenario 与 Evidence 满足所配置的 CI Policy，不代表“Agent 被永久认证为安全”。仓库同时提供 reusable GitHub Composite Action，并在 CI 中证明 secure mock target 可以通过、intentionally insecure target 必须被拒绝。
...
v1.0 定义 bounded Conformance Claim 格式，区分 **Control Conformance、Profile Conformance、Baseline Assessment**。Status 与 Assurance 必须分开：`PASS` 表示 Scope 内满足 Pass Criteria；`YAL-4` 表示证据深度达到对抗验证。Critical Control 的失败不能被其它 Pass 平均掉。

任何对外 Claim 应写清 YASC Version、精确 System State、Environment、Included/Excluded Path、Profile、逐 Control Result、Achieved YAL、Evidence/Assurance Case、Known Gap、Validity Window、Revalidation Trigger 和 Reviewer Independence。v1.0 本身仍不是商业 Certification Program，因此不能把一次评估宣传为“YASC certified secure”。

---

## 18. v1.0 的 58 个 Verification Test

v1.0 的测试集不把 Agent Security 等同于 Prompt Injection。测试面覆盖 Instruction、Authorization、Identity、Memory/Context、Execution、Approval、Observability、Resilience、Supply Chain、Temporal Behavior，以及语义等价变形。完整 Procedure、Expected Result、Evidence 与 Control Mapping 以 `schema/tests.yaml` 和 `verification-tests/` 为准。
...
有限次测试不能证明未知攻击不存在；模型可能非确定；外部服务会变化；Mapping 会过期；Checklist 不能替代实现质量；Privacy/Safety/Legal/行业风险可能需要额外控制；缺乏 Telemetry 的闭源系统可能无法达到高 YAL；v1.0 不是商业 Certification Program；一次 PASS 不能被宣传成永久或无边界的安全认证。

---

## 27. 后续研究方向

v1.0 之后可继续推进主流 Agent Framework Adapter、Target-specific Oracle/Assertion Package、Signed Evidence Attestation、Action Impact Classification、Portable Evidence Attestation、跨 Gateway 的 Policy Decision Interoperability、Agent Capability Graph Discovery、Stochastic Security Failure Quantification、Attack Corpus Governance、Multi-Agent Authority Conservation、Memory Provenance/Revocation、Agent Identity、MCP Conformance Profile、Production-safe Verification，以及未来 Certification 所需的 Assessor Competence / Independence Rules。
...
Git 仓库是 YASC 的真源。主要资产包括：

- `schema/controls.yaml` — Control 真源；
- `schema/tests.yaml` — Verification Test 真源；
- `schema/profiles.yaml` — Profile 真源；
- `schema/target.schema.json` — Agent/MCP Adapter 目标配置；
- `schema/execution-scenarios.schema.json` — 可执行 Scenario 格式；
- `schema/gate-policy.schema.json` — CI Security Gate 策略；
...
- `templates/target.yaml`、`templates/gate-policy.yaml`；
...
- `harness/yasc_harness/` — 可安装的 Agent/MCP Verification Harness；
- `harness/scenarios/core.yaml` — 覆盖 YAT Catalog 的 Scenario Registry；
- `scripts/yasc_harness.py` — 仓库内 CLI Wrapper；
- `.github/actions/yasc-security-gate/` — 可复用的 CI Security Gate Action；
- `docs/harness-architecture.md`；
- `docs/ci-security-gate.md`；
- `docs/validation-model.md`；
- `docs/evidence-model.md`；
- `docs/provenance.md`。

内置 Scenario Registry 会明确区分 **automated** 与 **guided**。`guided` 不代表 YAT 没有测试方法，而表示该规范测试依赖目标系统特有的身份、数据、真实副作用、故障注入或遥测条件，Reference Harness 不会用一个虚假的通用 Probe 冒充完整验证。可使用 `yasc scenario-init` 生成目标专用 Scenario 的起点。
